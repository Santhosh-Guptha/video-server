...
export function WebRTCPlayer({ streamId, posterLabel, isFocused, minimal, onFallbackToHls, onClose, onFocus }: WebRTCPlayerProps) {
  ...
  const stablePlaybackSinceRef = useRef(0);
  const preferH264Ref = useRef(false);
  ...
  const cleanupConnection = () => {
    ...
    if (pcRef.current) {
      ...
      const sessionUrl = stats.protocol === 'WHEP' ? (pc as any).sessionUrl : null;
      if (sessionUrl) {
  ...
  const startWebRTC = async () => {
    ...
    connectionTimeoutRef.current = window.setTimeout(() => {
      ...
      handleDisconnection('WebRTC produced no video before the connection timeout');
    }, recoverySettings.webrtc_connection_timeout_seconds * 1000);
    ...
    try {
      ...
      const pc = new RTCPeerConnection({
        ...
        // Prefer the fixed MediaMTX LAN/TCP candidate. TURN remains available
        // automatically when a device cannot reach the server directly.
      ...
      if (preferH264Ref.current && videoTransceiver.setCodecPreferences) {
        ...
        const compatible = codecs.filter(codec => codec.mimeType.toLowerCase() !== 'video/h265');
        if (compatible.some(codec => codec.mimeType.toLowerCase() === 'video/h264')) {
          videoTransceiver.setCodecPreferences(compatible);
        }
      }
      ...
      pc.onconnectionstatechange = () => {
        if (!isCurrent() || pcRef.current !== pc) return;
        if (pc.connectionState === 'failed' || pc.connectionState === 'closed') {
          handleDisconnection(`Peer connection ${pc.connectionState}`);
        }
      };
      ...
      if (!isCurrent()) {
        const staleLocation = whepResp.headers.get('Location');
        if (staleLocation) fetch(staleLocation, { method: 'DELETE' }).catch(() => {});
        pc.close(); return;
      }
  ...
  const startStatsInterval = () => {
    ...
    statsIntervalRef.current = window.setInterval(async () => {
      ...
      try {
        ...
        if (pcRef.current !== pc || !isMountedRef.current) return;
        ...
        if (videoTrackStats) {
          ...
          if (framesDelta > 0 || lastFrameAtRef.current === 0) {
            lastFrameAtRef.current = now;
          } else if (!videoRef.current?.paused && now - lastFrameAtRef.current >= recoverySettings.webrtc_stall_timeout_seconds * 1000) {
            // ICE may stay connected even after media stops. Recover a frozen
            // receiver rather than leaving a stale frame labelled LIVE.
            setLoaded(false);
            preferH264Ref.current = true;
            handleDisconnection(`No decoded video frames for ${recoverySettings.webrtc_stall_timeout_seconds} seconds`);
            return;
          }
          if (framesDelta > 0) {
            if (!stablePlaybackSinceRef.current) stablePlaybackSinceRef.current = now;
            if (now - stablePlaybackSinceRef.current >= 30000) reconnectCountRef.current = 0;
          } else {
            stablePlaybackSinceRef.current = 0;
          }
