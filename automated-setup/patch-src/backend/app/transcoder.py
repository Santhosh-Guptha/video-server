...
class TranscoderManager:
    ...
    async def ensure_transcoder(
        ...
        async with cls._stream_locks.setdefault(stream_id, asyncio.Lock()):
            state = cls._transcoders.get(stream_id)

            # If transcoder exists and is running, cancel any pending shutdown and reuse
            if state and state.process.returncode is not None:
            ...
            # Capacity check
            if len(cls._transcoders) + len(cls._starting) >= settings.max_active_transcoders:
            ...
            # Reserve capacity before awaiting MediaMTX or FFmpeg; other streams may start concurrently.
            cls._starting.add(stream_id)
            try:
            ...
            for _ in range(25): # 25 * 0.2s = 5s
                try:
                    resp = await _mtx_req("GET", f"{settings.mediamtx_api_url}/v3/paths/get/{h264_path}", timeout=1.0)
