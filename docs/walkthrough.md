...
3. **Frontend Integration (`frontend/src/pages/CameraManagement.tsx`)**:
   - Added a **"Remote Device ONVIF Configuration"** control section inside the edit camera form interface.
   - Allows users/admins to directly trigger hardware parameter updates (FPS, bitrate, resolution) on remote IP cameras over ONVIF or vendor-specific API fallbacks, updating the VMS database and auto-restarting ingress immediately.
   - Verified that Vite builds cleanly on VM 208 and hot-reloaded the configuration changes.
