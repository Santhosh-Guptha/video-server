        ...
        try:
            ...
            async def _download_task_impl(task):
                ...
                if output_path.exists() and output_path.stat().st_size > 0:
                    is_complete = False
                    try:
                        dur = await get_file_duration_async(str(output_path), recovery_probe_semaphore)
                ...
                codec_args = ["-c:v", "libx264", "-threads", "2", "-preset", "superfast", "-crf", "23", "-an"] if is_hevc else ["-c:v", "copy", "-an"]
        ...
        try:
            ...
            async for session in get_session():
                ...
                for stream in streams:
                    ...
                    # A zero/negative camera value means no camera-specific limit;
                    # the global storage cap still applies.
                    archive_days = min(archive_days, retention_cap) if archive_days > 0 else retention_cap
