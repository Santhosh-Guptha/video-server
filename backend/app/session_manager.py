...
async def webrtc_session_watchdog_cleanup(db_session: AsyncSession):
    ...
    try:
        try:
            response = await _mtx_request("GET", f"{settings.mediamtx_api_url}/v3/webrtcsessions/list", timeout=5.0)
        except Exception:
            response = None
        if response is None or response.status_code != 200:
            try:
                response = await _mtx_request("GET", f"{settings.mediamtx_api_url}/v3/webrtcsessions", timeout=5.0)
            except Exception:
                response = None
