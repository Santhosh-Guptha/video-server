...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('synced_from_api', sa.Boolean(), server_default='false', nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('profile_type', postgresql.ENUM('MAIN', 'SUB', 'MOBILE', name='profile_type_enum', create_type=False), nullable=False),
        ...
        sa.Column('stream_mode', sa.String(length=16), server_default='AUTO', nullable=False),
        sa.Column('stream_source', sa.String(length=16), nullable=True),
        sa.Column('last_push_seen', sa.DateTime(timezone=True), nullable=True),
        sa.Column('pull_failed_since', sa.DateTime(timezone=True), nullable=True),
        sa.Column('status', postgresql.ENUM('REGISTERED', 'CONNECTING', 'ONLINE', 'DEGRADED', 'RECONNECTING', 'OFFLINE', 'ERROR', name='stream_state_enum', create_type=False), server_default='REGISTERED', nullable=False),
